...
interface ThuocMSC {
  ...
  soNhaThauThamDu?: string;
  diaDiem?: string;
}

export default function ThuocMSCPage() {
  const [data, setData] = useState<ThuocMSC[]>([]);
  ...
  const [visibleColumns, setVisibleColumns] = useState<Set<string>>(new Set([
    ...
    'dangBaoChe', 'hanDung', 'tenCoSoSanXuat', 'nuocSanXuat', 'quyCachDongGoi', 
    'donViTinh', 'soLuong', 'donGia', 'nhomThuoc', 'maTBMT', 
    'tenChuDauTu', 'hinhThucLCNT', 'ngayDangTai', 'soQuyetDinh', 
    'ngayBanHanhQuyetDinh', 'soNhaThauThamDu', 'diaDiem'
  ]));
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/thuoc-msc?${params.toString()}`);
  ...
  const fetchStats = async () => {
    try {
      ...
      const res = await fetch(`/api/thuoc-msc/stats?${params.toString()}`);
      if (res.ok) {
        const result = await res.json();
        setStats(result);
      }
  ...
  const handleExportExcel = async () => {
    ...
    try {
      setExporting(true);
      const response = await fetch('/api/thuoc-msc/export', {
        method: 'POST',
        headers: {
  ...
  const columnDefinitions = [
    ...
    { key: 'maTBMT', label: 'Mã TBMT' },
    { key: 'tenChuDauTu', label: 'Tên Chủ đầu tư' },
    { key: 'hinhThucLCNT', label: 'Hình thức LCNT' },
    { key: 'ngayDangTai', label: 'Ngày đăng tải' },
    { key: 'soQuyetDinh', label: 'Số quyết định' },
    { key: 'ngayBanHanhQuyetDinh', label: 'Ngày ban hành QĐ' },
    { key: 'soNhaThauThamDu', label: 'Số nhà thầu tham dự' },
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <AdvancedSearch
        fields={[
          ...
          { value: "hanDung", label: "Hạn dùng", type: "string" },
          ...
          { value: "maTBMT", label: "Mã TBMT", type: "string" },
          { value: "tenChuDauTu", label: "Tên Chủ đầu tư", type: "string" },
          { value: "hinhThucLCNT", label: "Hình thức LCNT", type: "string" },
          { value: "ngayDangTai", label: "Ngày đăng tải", type: "date" },
          { value: "soQuyetDinh", label: "Số quyết định", type: "string" },
          { value: "ngayBanHanhQuyetDinh", label: "Ngày ban hành QĐ", type: "date" },
          { value: "soNhaThauThamDu", label: "Số nhà thầu tham dự", type: "string" },
          { value: "diaDiem", label: "Địa điểm", type: "string" },
        ]}
      ...
      {loading ? (
        ...
        <>
          <div className="relative mb-4">
            {/* Nút Ẩn/Hiện cột, Freeze Panes và Xuất Excel */}
                  ...
                  return (
                    ...
                    >
                      ...
                      {visibleCols.map((col, colIndex) => {
                        ...
                        } else if (colKey === 'ngayDangTai') {
                          cellContent = item.ngayDangTai
                            ? new Date(item.ngayDangTai).toLocaleDateString("vi-VN")
                            : "-";
                        } else if (colKey === 'ngayBanHanhQuyetDinh') {
                          cellContent = item.ngayBanHanhQuyetDinh
                            ? new Date(item.ngayBanHanhQuyetDinh).toLocaleDateString("vi-VN")
                            : "-";
                        } else {
