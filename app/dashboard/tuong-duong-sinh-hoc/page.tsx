...
export default function TuongDuongSinhHocPage() {
  const [data, setData] = useState<TuongDuongSinhHoc[]>([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`/api/tuong-duong-sinh-hoc?${params}`);
      if (res.ok) {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      setExporting(true);
      const response = await fetch('/api/tuong-duong-sinh-hoc/export', {
        method: 'POST',
        headers: {
      ...
      let filename = 'Tuong_Duong_Sinh_Hoc.xlsx';
              ...
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-bold text-red-600 uppercase min-w-[150px]">
                    {getColumnName('dangBaoChe', 'Dạng bào chế')}
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-red-600 uppercase min-w-[150px]">
                    {getColumnName('quyCachDongGoi', 'Quy cách đóng gói')}
                  </th>
                  <th className="px-4 py-3 text-left text-xs font-bold text-red-600 uppercase min-w-[150px]">
