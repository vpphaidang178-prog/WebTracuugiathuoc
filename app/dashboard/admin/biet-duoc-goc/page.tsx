...
export default function QuanLyBietDuocGocPage() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch('/api/import-history?type=BDG');
      if (res.ok) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('/api/biet-duoc-goc/import', {
        method: 'POST',
        body: formData,
      });
  ...
  const downloadTemplate = async () => {
    try {
      const res = await fetch('/api/biet-duoc-goc/template');
      const blob = await res.blob();
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'Template_Biet_Duoc_Goc.xlsx';
  ...
  const handleDeleteAll = async () => {
    const confirmed = window.confirm(
      '⚠️ CẢNH BÁO: Bạn có chắc chắn muốn xóa TẤT CẢ dữ liệu Biệt dược gốc?\n\n' +
    ...
    try {
      const res = await fetch('/api/biet-duoc-goc', {
        method: 'DELETE',
      });
      ...
      setMessage({
        type: 'success',
        text: result.message || `Đã xóa ${result.deletedCount || 0} bản ghi biệt dược gốc`
      });
  ...
  const defaultColumnNames: { [key: string]: string } = {
    ...
    tenThuoc: 'Tên thuốc',
    hamLuong: 'Hàm lượng',
    dangBaoCheQuyCach: 'Dạng bào chế/Quy cách',
    soDangKy: 'Số đăng ký',
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">
        Quản lý Danh mục Biệt dược gốc - Import từ Excel
      </h2>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        ...
        <ol className="list-decimal list-inside space-y-1 text-sm text-blue-800">
          <li>File Excel phải có đúng 8 cột theo thứ tự bên dưới</li>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
        <h3 className="font-semibold text-gray-900 mb-2">
          8 cột theo thứ tự (Cột A → H):
        </h3>
        <div className="grid grid-cols-2 gap-2 text-sm text-gray-700">
          <div>1. Tên thuốc *</div>
          <div>2. Hàm lượng</div>
          <div>3. Dạng bào chế/Quy cách</div>
          <div>4. Số đăng ký</div>
          <div>5. Cơ sở sản xuất</div>
          <div>6. Địa chỉ cơ sở sản xuất</div>
          <div>7. Ghi chú</div>
          <div>8. Số quyết định</div>
      ...
      {showColumnConfig && (
        <div className="mb-6 bg-purple-50 border border-purple-200 rounded-lg p-6">
          <h3 className="text-xl font-bold text-purple-900 mb-4">
            ⚙️ Quản lý tên cột - Biệt dược gốc
          </h3>
          <p className="text-sm text-purple-700 mb-4">
            Thay đổi tên hiển thị của các cột trong bảng "Biệt dược gốc"
          </p>
